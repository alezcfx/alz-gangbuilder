    ...
    MySQL.Async.fetchScalar('SELECT COUNT(*) FROM gangs WHERE name = @name', {
        ...
        if count > 0 then
            MySQL.Async.execute([[
                ...
                ['@boss_menu_pos'] = gangData.bossMenuPos and json.encode(gangData.bossMenuPos) or nil
            }, function()
                ...
                if gangData.bossMenuPos then
                    SetPlayerGang(xPlayer, gangData.name, #gangData.grades - 1, function(success, message)
                        if success then
                            TriggerClientEvent('gangbuilder:setGangBoss', source, gangData.name, #gangData.grades - 1, gangData.bossMenuPos)
                        else
                            print("^1[DEBUG] Failed to set creator as gang boss:", message)
                        end
                    end)
                end
            end)
...
MySQL.ready(function()
    ...
    MySQL.Async.execute([[
        ...
        ADD COLUMN IF NOT EXISTS boss_menu_pos TEXT
    ]])
    ...
    MySQL.Async.fetchAll('SELECT * FROM gangs WHERE name = @name', {
        ...
        if #gangs > 0 then
            ...
            MySQL.Async.execute('UPDATE users SET gang = @gang, gang_grade = @grade WHERE identifier = @identifier', {
                ...
                targetPlayer.setJob(gangName, grade)
                
                MySQL.Async.fetchAll('SELECT boss_menu_pos FROM gangs WHERE name = @name', {
                    ['@name'] = gangName
                }, function(result)
                    if result[1] and result[1].boss_menu_pos then
                        TriggerClientEvent('gangbuilder:setGangBoss', targetPlayer.source, gangName, grade, result[1].boss_menu_pos)
                    end
                end)
...
ESX.RegisterServerCallback('gangbuilder:getPlayerGang', function(source, cb)
    ...
    if xPlayer then
        MySQL.Async.fetchAll('SELECT gang, gang_grade FROM users WHERE identifier = @identifier', {
            ...
            if result[1] and result[1].gang and result[1].gang ~= "aucun" then
                cb(result[1].gang, result[1].gang_grade)
            else
                cb(nil, 0)
            end
        end)
